...
def train_complete_pipeline():
    """
    FAST training pipeline with subset of data
    """

    print("=" * 60)
    print("GRAPH-BASED TAX FRAUD DETECTION - FAST TRAINING")
    print("=" * 60)
    ...
    # Keep all fraud cases + sample of legitimate transactions
    fraud_df = df[df["isFraud"] == 1]
    legit_df = df[df["isFraud"] == 0].sample(n=min(200000, len(df)), random_state=42)

    df = pd.concat([fraud_df, legit_df]).sample(frac=1, random_state=42)  # Shuffle

    print(f"Using subset: {len(df)} transactions")
    print(f"  - Fraud: {len(fraud_df)}")
    print(f"  - Legitimate: {len(legit_df)}")
    print(f"  - Fraud ratio: {len(fraud_df)/len(df)*100:.2f}%")
    ...
    history = trainer.train(
        data=data, num_epochs=100, early_stopping_patience=15  # Reduced from 200
    )
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("\nNext steps:")
    print("1. Run 'python app_enhanced.py' to start the web interface")
    print("2. Visit http://localhost:5002/dashboard")
    print("=" * 60)
